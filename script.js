...
(function () {
    ...
    var I18N = {
        ua: {
            ...
            "booking.sending": "Надсилаємо…",
            "booking.failed": "Не вдалося надіслати заявку. Перевірте зв’язок і спробуйте ще раз або зателефонуйте нам.",
    ...
    */
    ...
    /* ---- Appointments ------------------------------------------------------
       One POST to PostgREST. `Prefer: return=minimal` keeps the reply empty:
       we do not need the row back, and an insert-only policy has no reason to
       grant a read. */
    function supabaseInsert(table, row) {
        if (!window.fetch) return Promise.reject(new Error("fetch unavailable"));

        return fetch(SUPABASE.url + "/rest/v1/" + encodeURIComponent(table), {
            method: "POST",
            headers: {
                "apikey": SUPABASE.key,
            ...
            /* PostgREST puts the reason in the body — a blocked policy, an
               unknown column, a failed constraint. Surface it rather than a
               bare status, or the next person debugging this is guessing. */
    ...
    var AppointmentStore = {
        save: function (row) {
            return supabaseInsert(SUPABASE.table, row).catch(function (err) {
                /* Keep a copy on the device so a booking is never simply lost.
                   It is not resent automatically: a failed request may still
                   have reached the table, and a double booking is its own
                   problem. This is a record to recover from, not a queue. */
                try {
                    var kept = store("monarch_bookings_failed") || [];
                    if (!Array.isArray(kept)) kept = [];
                    kept.push({ row: row, at: new Date().toISOString(), error: String(err.message || err) });
                    store("monarch_bookings_failed", kept.slice(-20));
                } catch (e) { /* storage full or blocked — nothing else to try */ }
                throw err;
            });
        }
    };
    ...
    /* ---- Supabase ----------------------------------------------------------
       Booking requests are written straight to the `appointments` table over
       PostgREST. No client library: one fetch does the whole job, and the site
       stays a set of static files with no build step.

       The publishable key is meant to ship in the page — it identifies the
       project, it does not grant anything. What a visitor may actually do is
       decided by the row-level security policies on the table, so those have
       to allow an insert and nothing else. Without them this table is readable
       by anyone who opens the site, and it holds customer names and phone
       numbers. See the note in README-supabase.md. */
    var SUPABASE = {
        url: "https://mvnmnhtcgbgpbocjafqz.supabase.co",
        key: "sb_publishable_f8cgj01CBVrdHXrfAPiarA_RZzHC5VD",
        table: "appointments"
    };
    ...
    function submitBooking(el) {
        ...
        /* The service select carries the id; the table is read by a person, so
           it gets the name. Ukrainian either way — one language in the table
           beats rows that depend on which flag the customer had selected. */
        var picked = SERVICES.filter(function (s) { return s.id === el.service.value; })[0];

        var row = {
            name: el.name.value.trim(),
            phone: phoneE164(phoneCountryFor(el.phone.id), el.phone.value),
        ...
        AppointmentStore.save(row).then(function () {
            if (el.success) {
            ...
            /* Do not claim a booking that never landed. The form keeps what was
               typed so the customer can retry without filling it in again. */
            if (window.console && console.error) console.error("[booking]", err);
            toast(t("booking.failed"));
            if (el.success) {
                el.success.textContent = t("booking.failed");
                el.success.hidden = false;
            }
        }).then(function () {
            if (submit) {
